...
function activeInMonth(p, mIdx, yr) {
  if (!yr) yr = today().getFullYear();
  const mDate = new Date(yr, mIdx, 1);
  if (p.contractEnd) {
    const e = new Date(p.contractEnd);
    if (mDate > new Date(e.getFullYear(), e.getMonth(), 1)) return false;
  }
  if (p.contractStart) {
    const s = new Date(p.contractStart);
    if (mDate < new Date(s.getFullYear(), s.getMonth(), 1)) return false;
  }
  if (p.interval === "einmalig") return false;
  if (p.interval === "viertelj.") {
    const ref = p.contractStart ? new Date(p.contractStart).getMonth() : 0;
...
/** Betrag des Postens in Monat mIdx, optionales Jahr */
